...
import { createStUiState } from "./state.js";
import { stApi } from "./api.js";
import { StShell } from "./StShell.js";
...
function ChatNavRow(props) {
    return (_jsx("button", { type: "button", className: props.panel === 'chat' ? css.navBtnActive : css.navBtn, onClick: () => { props.select('chat'); }, children: "\uD83D\uDCAC \u5BF9\u8BDD" }));
}
/** The nav row for the chat-management surface. */
function ChatsNavRow(props) {
    return (_jsx("button", { type: "button", className: props.panel === 'chats' ? css.navBtnActive : css.navBtn, onClick: () => { props.select('chats'); }, children: "\uD83D\uDDC2 \u804A\u5929" }));
}
/** The nav row for the group-management surface. */
function GroupsNavRow(props) {
    return (_jsx("button", { type: "button", className: props.panel === 'groups' ? css.navBtnActive : css.navBtn, onClick: () => { props.select('groups'); }, children: "\uD83D\uDC6A \u7FA4\u804A" }));
}
/**
...
export function apply(ctx) {
    ...
    const face = { hooks: { st: source }, api: stApi, actions };

...
import { validateApiConfig, API_SOURCES } from '@deepseek-ai/dsh-st-api-config';
...
const routes = {
    ...
    'POST chats/create': async (ctx, req, res) => {
        const body = await readJson(req);
        const chatId = await ctx.stChat.create(String(body.avatar ?? ''), String(body.userName ?? 'User'), String(body.characterName ?? ''), typeof body.firstMessage === 'string' ? body.firstMessage : undefined);
    ...
    'POST groups/create': async (ctx, req, res) => {
        ...
        const input = { name: body.name, ...(members === undefined ? {} : { members }) };
    ...
    'POST groups/next-speaker': async (ctx, req, res) => {
        ...
        const character_id = await ctx.stGroup.selectNextSpeaker(String(body.id ?? ''), lastSpeakerId, chatMessages);
    ...
    'POST generate': async (ctx, req, res) => {
        ...
        const worldNames = Array.isArray(body.world)
            ? body.world.map(String).filter((w) => w.length > 0)
        ...
        const contextTemplate = typeof body.storyString === 'string' && body.storyString.length > 0
        ...
        const override = Array.isArray(body.messages) ? body.messages : undefined;
        ...
        if (typeof body.presetId === 'string' && body.presetId.length > 0) {
            ...
            if (preset.generation.temp > 0)
                temperature = preset.generation.temp;
            if (preset.generation.max_tokens > 0)
                maxTokens = preset.generation.max_tokens;
            if (preset.generation.top_p > 0)
                topP = preset.generation.top_p;
            if (preset.generation.top_k > 0)
                topK = preset.generation.top_k;
            if (preset.generation.min_p > 0)
                minP = preset.generation.min_p;
            if (preset.generation.frequency_penalty !== 0)
                frequencyPenalty = preset.generation.frequency_penalty;
            if (preset.generation.presence_penalty !== 0)
                presencePenalty = preset.generation.presence_penalty;
            if (preset.generation.repetition_penalty !== 1)
                repetitionPenalty = preset.generation.repetition_penalty;
            if (preset.generation.seed !== 0)
                seed = preset.generation.seed;
            if (preset.generation.stop_sequences.length > 0)
                stopSequences = preset.generation.stop_sequences;
        ...
        if (maxContextTokens === undefined && typeof body.presetId === 'string' && body.presetId.length > 0) {
            const preset = await ctx.stPreset.get(body.presetId);
            const presetMax = preset?.extensions?.max_context;
            if (typeof presetMax === 'number' && presetMax > 0)
                maxContextTokens = Math.floor(presetMax);
        }
        ...
        if (typeof cardWorld === 'string' && cardWorld.length > 0 && !worldNames.includes(cardWorld)) {
            ...
            if (book !== undefined)
                books.push({ name: cardWorld, file: book });
        }
        ...
        if (books.length > 0) {
            ...
            if (hasVectorized) {
                vectorHits = new Map((await ctx.stVector.searchWorld(chat.messages.slice(-10).map((m) => m.mes).join('\n'), { topK: 5 }))
                    .map((hit) => [hit.key, hit.score]));
            }
            const scanOptions = { timedState: wiTimedState, nowMs: Date.now() };
            if (vectorHits !== undefined)
                scanOptions.vectorHits = vectorHits;
            if (typeof body.worldInfoDepth === 'number' && body.worldInfoDepth >= 1) {
                scanOptions.scanDepthMessages = Math.floor(body.worldInfoDepth);
            }
            if (typeof body.worldInfoCaseSensitive === 'boolean')
                scanOptions.caseSensitive = body.worldInfoCaseSensitive;
            if (typeof body.worldInfoMatchWholeWords === 'boolean')
                scanOptions.matchWholeWords = body.worldInfoMatchWholeWords;
            if (body.worldInfoRecursive === false)
                scanOptions.maxRecursionSteps = 0;
            if (maxContextTokens !== undefined) {
                scanOptions.tokenBudget = Math.floor(maxContextTokens * wiBudgetPercent / 100);
            }
            const activated = scanWorldInfo(books, {

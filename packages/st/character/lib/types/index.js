...
export function getUniqueName(baseName, exists, startIndex = 1, maxTries = 1000) {
    if (!exists(baseName))
        return baseName;
 ...
 */
export function readCardFromPng(png) {
 ...
 */
export function writeCardToPng(png, cardJson) {
 ...
 */
export class StCharacterService extends Service {
    constructor(ctx) {
...
class StCharacterFileProvider extends StCharacterService {
    ...
    async rename(avatar, newName) {
        ...
        if (existsSync(chatsOld) && !existsSync(chatsNew))
            await rename(chatsOld, chatsNew);
        return `${name}.png`;
    }
    async delete(avatar) {
        const path = this.avatarPath(avatar);

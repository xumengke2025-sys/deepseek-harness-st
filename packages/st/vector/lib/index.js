...
const LOCAL_EMBEDDING_DIMS = 512;
...
function tokenize(text) {
	const tokens = [];
	for (const word of text.toLowerCase().match(/[\p{L}\p{N}]+/gu) ?? []) if (CJK.test(word)) tokens.push(...word.match(/./gu) ?? []);
...
var OpenAIEmbeddingProvider = class {
	opts;
	resolveCredential;
	constructor(opts, resolveCredential = async () => void 0) {
		this.opts = opts;
	...
	async embed(texts) {
		...
		const rows = Array.from({ length: texts.length }, () => void 0);
		for (const item of body.data ?? []) if (typeof item.index === "number" && Array.isArray(item.embedding)) rows[item.index] = l2normalize(item.embedding);
		if (rows.some((row) => row === void 0)) throw new Error("st-vector embeddings endpoint: response is missing rows for some inputs");
		return rows;
	}
};
...
const DATA_BANK_CHUNK_CHARS = 1e3;
/**
...
*/
function chunkText(text, size = DATA_BANK_CHUNK_CHARS) {
	...
	for (const paragraph of paragraphs) {
		if (current.length > 0 && current.length + paragraph.length + 1 > size) {
			chunks.push(current);
			current = paragraph;
		} else current = current.length > 0 ? `${current}\n${paragraph}` : paragraph;
...
*/
var StVectorService = class extends Service {
	constructor(ctx) {
		super(ctx, "stVector");
	}
};
...
*/
var StVectorLocalProvider = class extends StVectorService {
	filesDir;
	vectorsDir;
	embedder;
	...
	constructor(ctx, config) {
		...
		this.embedder = config.embedding === void 0 ? new LocalNgramEmbedder(config.dims) : new OpenAIEmbeddingProvider(config.embedding, async (ref) => {
	...
	search(queryVector, namespace, threshold, topK) {
		const hits = [];
		for (const rows of namespace) for (const [key, row] of rows) {
			if (row.vector.length !== queryVector.length) throw new Error(`st-vector: index row ${key} has ${row.vector.length} dims but the query has ${queryVector.length}; re-index after switching the embedding provider`);
			...
			if (row.text !== void 0) hit.text = row.text;
			hits.push(hit);
		}
	...
	async searchWorld(query, options = {}) {
		...
		for (const stem of await this.indexStems("worlds")) if (!this.worlds.has(stem)) this.worlds.set(stem, await this.loadIndex("worlds", stem));
		return this.search(queryVector, this.worlds.values(), options.threshold ?? .35, options.topK ?? Number.POSITIVE_INFINITY).map(({ key, score }) => ({
			key,
			score
		}));
	}
	async indexFile(name, text) {
		...
		const rows = /* @__PURE__ */ new Map();
		for (const [i, chunk] of chunks.entries()) rows.set(`${name}#${i}`, {
			vector: vectors[i],
			text: chunk
		});
	...
	async listFiles() {
		...
		return (await readdir(dir)).filter((f) => f.toLowerCase().endsWith(".json")).map((f) => f.slice(0, -5)).sort((a, b) => a.localeCompare(b));
	}
	async searchFiles(query, options = {}) {
		const queryVector = await this.one(query);
		for (const stem of await this.indexStems("bank")) if (!this.bank.has(stem)) this.bank.set(stem, await this.loadIndex("bank", stem));
		return this.search(queryVector, this.bank.values(), options.threshold ?? .05, options.topK ?? 5).filter((h) => typeof h.text === "string");
	}
	/** Persisted index stems in one scope, creating nothing. */
	async indexStems(scope) {
		const dir = join(this.vectorsDir, scope);
		if (!existsSync(dir)) return [];
		return (await readdir(dir)).filter((f) => f.toLowerCase().endsWith(".json")).map((f) => f.slice(0, -5));
	}
};
const name = "st-vector-local";
//#endregion
export { DATA_BANK_CHUNK_CHARS, LOCAL_EMBEDDING_DIMS, LocalNgramEmbedder, OpenAIEmbeddingProvider, StVectorService, chunkText, cosineSimilarity, StVectorLocalProvider as default, name };

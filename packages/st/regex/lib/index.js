...
function substituteMacros(text, ctx) {
	return text.replace(/\{\{char\}\}/gi, ctx.char).replace(/\{\{user\}\}/gi, ctx.user);
}
...
*/
function applyRegexScripts(scripts, text, target, macros = {
	char: "",
	user: ""
}) {
	let out = text;
...
var StRegexService = class extends Service {
	constructor(ctx) {
		super(ctx, "stRegex");
	}
};
var StRegexFileProvider = class extends StRegexService {
	config;
	static inject = [];
...
const name = "st-regex-file";
//#endregion
export { PLACEMENT, StRegexService, applyRegexScripts, StRegexFileProvider as default, name, scriptTargets };

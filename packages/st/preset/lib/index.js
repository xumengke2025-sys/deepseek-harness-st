...
*/
var StPresetService = class extends Service {
	constructor(ctx) {
		super(ctx, "stPreset");
	}
};
...
function normalizeStLegacyPreset(raw, id, name) {
	...
	for (const prompt of raw.prompts ?? []) if (prompt.identifier !== void 0) byIdentifier.set(prompt.identifier, prompt);
	const entries = ((raw.prompt_order?.find((row) => row.character_id === 100001) ?? raw.prompt_order?.[0])?.order ?? []).flatMap(({ identifier, enabled }) => {
...
function parsePresetFile(text, id) {
	...
	return {
		...preset,
		id,
		...preset.name === void 0 ? { name: id } : {}
	};
}
var FilePresetProvider = class extends StPresetService {
	root;
	...
	async list() {
		...
		for (const f of files) {
			...
			try {
				presets.push(parsePresetFile(await readFile(join(this.root, f), "utf-8"), f.slice(0, -5)));
			} catch {}
		}
	...
	async update(id, input) {
		...
		if (preset === void 0) throw new Error(`Preset ${id} not found`);
		if (input.name !== void 0) preset.name = input.name;
		if (input.description !== void 0) preset.description = input.description;
		if (input.apiSource !== void 0) preset.apiSource = input.apiSource;
		...
		if (input.mainPrompt !== void 0) preset.mainPrompt = input.mainPrompt;
		if (input.nsfw !== void 0) preset.nsfw = input.nsfw;
		if (input.jailbreakPrompt !== void 0) preset.jailbreakPrompt = input.jailbreakPrompt;
		if (input.extensions) Object.assign(preset.extensions, input.extensions);
		preset.modifyDate = (/* @__PURE__ */ new Date()).toISOString();
	...
	async importJson(json) {
		...
		if (isStLegacyPreset(parsed)) {
			const name = typeof parsed.preset_name === "string" && parsed.preset_name.length > 0 ? parsed.preset_name : "Imported Preset";
		...
		return this.create({
			...
			...apiSource !== void 0 ? { apiSource } : {},
			...generation !== void 0 ? { generation } : {},
			...instruct !== void 0 ? { instruct } : {},
			...promptOrder !== void 0 ? { promptOrder } : {},
			...mainPrompt !== void 0 ? { mainPrompt } : {},
			...nsfw !== void 0 ? { nsfw } : {},
			...jailbreakPrompt !== void 0 ? { jailbreakPrompt } : {}
		});
	}
	async exportJson(id) {
		const preset = await this.get(id);
	...
	async buildPrompt(presetId, characterData) {
		...
		const parts = [];
		if (preset.mainPrompt) parts.push(expandMacros(preset.mainPrompt, vars));
		...
		if (preset.instruct.enabled) {
			...
			return expandMacros(preset.instruct.systemPrompt, {
				...vars,
...
const name = "st-preset-file";
//#endregion
export { DEFAULT_GENERATION, DEFAULT_INSTRUCT, StPresetService, FilePresetProvider as default, name };
